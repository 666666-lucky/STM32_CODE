...
void TFTLCD_FSMC_Init(void)
{
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	...
	FSMC_ReadTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_B;	 //模式A 
...
#endif
#if 1
		FSMC_WriteTimingInitStructure.FSMC_AddressSetupTime =0x00;// 0x15;	 //地址建立时间（ADDSET）为16个HCLK  
  	FSMC_WriteTimingInitStructure.FSMC_AddressHoldTime = 0x00;//0x15;	 //地址保持时间		
  	FSMC_WriteTimingInitStructure.FSMC_DataSetupTime = 0x03;//0x05;		 //数据保存时间为6个HCLK	
  	FSMC_WriteTimingInitStructure.FSMC_BusTurnAroundDuration = 0x00;
  	FSMC_WriteTimingInitStructure.FSMC_CLKDivision = 0x00;
  	FSMC_WriteTimingInitStructure.FSMC_DataLatency = 0x00;
  	FSMC_WriteTimingInitStructure.FSMC_AccessMode = FSMC_AccessMode_B;	 //模式A  
#endif
...
void TFTLCD_Init(void)
{
	...
	tftlcd_data.id<<=8;
	tftlcd_data.id|=TFTLCD->LCD_DATA;  //41
#endif
	...
	LCD_WriteCmd(0xCF);  
	LCD_WriteData (0x00); 
	LCD_WriteData (0xa2);  //(0xC1);
	LCD_WriteData (0Xf0);  // (0X30);
	 
	LCD_WriteCmd(0xE8);  
	LCD_WriteData (0x84); //(0x85); 	 //85
	LCD_WriteData (0x11); //(0x10); 	 //00
	LCD_WriteData (0x7a); 	 //79
	
	
	LCD_WriteCmd(0xEA);  
	LCD_WriteData (0x66); //(0x00);  //00
	LCD_WriteData (0x00); 
	
	LCD_WriteCmd(0xED);  
	LCD_WriteData (0x55); //(0x64); 	//64
	LCD_WriteData (0x01); //(0x03); 	//03
	LCD_WriteData (0X23);//(0X12); 	//12
	LCD_WriteData (0X01);//(0X81); 	//81
	...
	LCD_WriteData (0x13);//(0x1B);   //SAP[2:0];BT[3:0] 	//13
	 
	LCD_WriteCmd(0xC5);    //VCM control 
	LCD_WriteData (0x23); //(0x30);
	LCD_WriteData (0x3F); //(0x30);
	 
	LCD_WriteCmd(0xC7);    //VCM control2 
	LCD_WriteData (0xA5);// (0XB7);
	
	LCD_WriteCmd(0xB1);  
	LCD_WriteData (0x00); 
	LCD_WriteData (0x17);//(0x1A); 
